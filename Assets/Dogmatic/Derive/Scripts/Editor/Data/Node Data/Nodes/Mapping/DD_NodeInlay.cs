...
{
    [Serializable]
    public class DD_NodeInlay : DD_NodeBase
    {
        #region public variables
        public int m_tilingX = 1;
        public int m_tilingY = 1;

        public int m_offsetX = 0;
        public int m_offsetY = 0;

        public int m_extentX = 1;
        public int m_extentY = 1;
        #endregion

        #region private variables
        bool m_showOutput = true;
        bool m_showInlaySettings = true;
        #endregion
        ...
        public override void DrawProperties()
        {
            ...
            DD_GUILayOut.TitleLabel(NodeType.Inlay.ToString());
            ...
            m_showInlaySettings = DD_GUILayOut.FoldOut(m_showInlaySettings, "Inlay Settings");

            if (m_showInlaySettings)
            {
                EditorGUI.BeginChangeCheck();
                DD_GUILayOut.Label("Tiling");
                m_tilingX = (int)DD_GUILayOut.Slider("X", (float)m_tilingX, 0, 10);
                m_tilingY = (int)DD_GUILayOut.Slider("Y", (float)m_tilingY, 0, 10);
                EditorGUILayout.Space(10);

                DD_GUILayOut.Label("Offset");
                m_offsetX = (int)DD_GUILayOut.Slider("X", (float)m_offsetX, 0, m_tilingX-1);
                m_offsetY = (int)DD_GUILayOut.Slider("Y", (float)m_offsetY, 0, m_tilingY-1);
                EditorGUILayout.Space(10);

                DD_GUILayOut.Label("Extent");
                m_extentX = (int)DD_GUILayOut.Slider("X", (float)m_extentX, 0, m_tilingX);
                m_extentY = (int)DD_GUILayOut.Slider("Y", (float)m_extentY, 0, m_tilingY);
                EditorGUILayout.Space(10);
        ...
        void Perform()
        {
            ...
            else
            {
                ...
                DD_NodeUtils.Inlay(inputTexture, m_outputs[0].outputTexture, m_tilingX, m_tilingY, m_offsetX, -(m_tilingY - m_offsetY), m_extentX, m_extentY);
